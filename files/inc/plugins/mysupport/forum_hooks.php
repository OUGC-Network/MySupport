...
use function MySupport\Core\_forums;
...
use function MySupport\Core\mysupport_change_assign;
use function MySupport\Core\mysupport_change_category;
use function MySupport\Core\mysupport_change_priority;
use function MySupport\Core\mysupport_mod_log_action;
use function MySupport\Core\mysupport_redirect_message;
use function MySupport\Core\mysupport_update_points;
    ...
    if (!empty($mysupport_cache['priorities'])) {
        ...
        foreach ($mysupport_cache['priorities'] as $priority) {
            ...
            if (!empty($priority['extra'])) {
                $mysupport_priority_classes .= '.mysupport_priority_' . strtolower(
...
function forumdisplay_thread(): bool
{
    ...
    if ($foruminfo['mysupport']) {
        if ($thread['priority'] != 0 && $thread['visible'] == 1) {
            $priority_class = ' mysupport_priority_' . htmlspecialchars_uni(
                    str_replace(' ', '_', $priorities[$thread['priority']])
                );
        }
        ...
        $mysupport_status = _get_display_status(
            $thread['status'],
            $thread['onhold'],
            $thread['statustime'],
            $thread['uid']
...
function search_results_thread(): bool
{
    forumdisplay_thread();
...
function global_intermediate(): bool
{
    ...
    // is it enabled?
    if (isTechnicalStatusEnabled() && $mybb->settings['mysupport_technicalnotice'] != 'off') {
            ...
            if ($mybb->settings['mysupport_technicalnotice'] == 'global') {
                ...
                if (!empty($fid) && $technical_count_forum > 0) {
                    $notice_text = $lang->sprintf(
                        ...
                        intval($technical_count_global),
                        $threads_text,
                        intval($technical_count_forum)
                    );
...
function usercp_menu_built09(): bool
{
    modcp_start09();
            ...
            if ((THIS_SCRIPT == 'usercp.php' && $mybb->get_input(
                ...
                if (THIS_SCRIPT == 'modcp.php') {
                    ...
                    $query = $db->simple_select(
                        'threads',
                        'status',
                        "uid = '{$mybb->user['uid']}'{$stats_in_sql}{$stats_where_sql}"
                    );
                }
                if ($db->num_rows($query) > 0) {
        ...
        if ($db->num_rows($query) == 0) {
            ...
            while ($thread = $db->fetch_array($query)) {
                ...
                if ($thread['priority'] != 0) {
                    $priority_class = " class=\"mysupport_priority_" . strtolower(
                            htmlspecialchars_uni(
                                str_replace(
                                    ' ',
                                    '_',
                ...
                $thread['forumlink'] = "<a href=\"" . get_forum_link($thread['fid']) . "\">" . htmlspecialchars_uni(
                        $thread['name']
                    ) . '</a>';
...
function moderation_start(): bool
{
    global $mybb;
    ...
    if ($type == 'search') {
        ...
        $query = $db->simple_select(
            'threads',
            'tid',
            'fid IN (' . $db->escape_string($mysupport_forums) . ') AND tid IN (' . $db->escape_string(
                $tids
    ...
    if (strpos($mybb->get_input('action'), 'onhold') !== false) {
        ...
        if ($category == 0) {
            ...
            $categories = get_categories($forum);
...
function newreply_start(): bool
{
    ...
    if ($forum['mysupport']) {
        if ($mybb->settings['mysupport_bumpnotice']) {
            if ($thread['status'] == 1 && $thread['uid'] != $mybb->user['uid'] && !($mybb->usergroup['canmarksolved'] || is_moderator(
                        $forum['fid'],
                        '',
                        $post['uid'] ?? 0
                    ))) {
...
function newthread_do_newthread_end(): bool
{
    global $mybb;
    ...
    if ($forum['mysupport']) {
        ...
        if ($post['visible'] == 1) {
            if ($forum['allowbestanswerstatus']) {
                ...
                if ($mybb->user['uid'] == $thread['uid'] && $mybb->usergroup['canmarkbestanswer'] || is_moderator(
                        $forum['fid'],
                        'canmarkbestanswer'
                    )) {
                    if ($thread['bestanswer'] == $post['pid']) {
            ...
            if (empty($post['mysupport_bestanswer_highlight'])) {
                if ($mybb->settings['mysupport_highlightstaffposts']) {
                    $usergroup = get_usergroup_permissions(false, $post);
        ...
        if ($mybb->settings['mysupport_enablesupportdenial'] && $forum['mysupportdenial']) {
            ...
            if ($post['deniedsupport'] == 1) {
                ...
                if ($mybb->usergroup['canmanagesupportdenial']) {
                    ...
                    $post['mysupport_deny_support_post'] = eval(
                    $templates->render(
        ...
        if ($thread['issupportthread'] == 1 && $thread['firstpost'] == $post['pid']) {
            $post['mysupport_status'] = _get_display_status(
                $thread['status'],
                $thread['onhold'],
                $thread['statustime'],
                $thread['uid']
...
function showthread_start20(): bool
{
    global $mybb;
    ...
    if (!empty($forum['mysupport']) && $mybb->get_input('action') != 'mysupport' && $mybb->get_input(
            'action'
        ) != 'bestanswer') {
        ...
        if ($thread['issupportthread'] == 1) {
            ...
            if ($count > 0) {
                $current_status = _get_friendly_status($thread['status']);
                ...
                if ($mybb->get_input('mysupport_full')) {
                    ...
                    if (!empty($categories)) {
                        ...
                        foreach ($categories as $category_id => $category) {
                            ...
                            $categories_list .= "<option value=\"" . intval(
                                    $category_id
                                ) . "\"{$selected}>" . htmlspecialchars_uni($category) . "</option>\n";
                            ++$count;
                        }
                        if ($thread['prefix'] != 0) {
    ...
    if ($mybb->get_input('action') == 'mysupport') {
        ...
        if (($mybb->get_input(
                'via_form',
                MyBB::INPUT_INT
            ) && isset($mybb->input['issupportthread']) && (($issupportthread == 1 && $old_issupportthread == 0) || (!$issupportthread && $old_issupportthread == 1)))) {
