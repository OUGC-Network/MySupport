...
function get_assign_users()
{
	...
	if(!empty($assign_groups))
	{
		...
		$query = $db->simple_select(
			"users",
			"uid, username",
			"usergroup IN (".$db->escape_string($assigngroups).") OR displaygroup IN (".$db->escape_string($assigngroups).") OR {$assign_concat_sql}",
			[
...
**/
function _change_hold($thread_info, $onhold = 0, $multiple = false)
{
 ...
 * @param bool If this is changing the priority of multiple threads.
**/
function change_issupportthread($thread_info, $issupportthread, $multiple = false)
{
	global $db, $lang;
 ...
 *
**/
function recount_technical_threads()
{
	global $db, $cache;
...
function isTechnicalStatusEnabled()
{
	$isEnabled = null;
