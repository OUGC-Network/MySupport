 ...
 **/
function friendlyOpenStatusGet(int $status = 0): string
{
    global $lang;
    ...
    return match ($status) {
        3, 1 => $lang->mySupportStatusTextSolved,
        2 => $lang->mySupportStatusTextTechnical,
        4 => $lang->mySupportStatusTextNotTechnical,
 ...
 * @return string
 */
function displayStatusGet(array $threadData, ?int $userID = null): string
{
 ...
 */
function threadOpenStatusUpdate(int $threadID, int $status = ThreadStatus::Solved): void
{
    global $mybb;

    $currentUserID = (int)$mybb->user['uid'];

    if ($status === ThreadStatus::Solved) {
        threadUpdate([
...
function threadTechnicalStatusUpdate(int $threadID, int $technicalStatus = ThreadStatus::Technical): void
{
